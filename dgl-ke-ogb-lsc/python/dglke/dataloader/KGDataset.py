...
class KGDatasetWiki(KGDataset):
    ...
    def __init__(self, path, args,name='wikikg90m', test_mode='test-dev'):
        self.name = name
        self.dataset = WikiKG90Mv2Dataset(path)
        #self.train = self.dataset.train_hrt.T
        self.train = np.load(args.trainhrt_path).T
        ...
        self.valid_dict['h,r->t']['t_candidate'] = np.load(args.val_candidate_path)
        self.test_dict = self.dataset.test_dict(test_mode)
        print(self.test_dict)
        if test_mode == 'test-dev':
            self.test_dict['h,r->t']['t_candidate'] = np.load('/train14/sli/jswang19/OGB/wikikg90m-v2/merge_candidate/TD_PIE6w_RM32.npy')
        else:
            self.test_dict['h,r->t']['t_candidate'] = np.load('/train14/sli/jswang19/OGB/wikikg90m-v2/merge_candidate/TC_PIE6w_RM32.npy')
...
def get_dataset(data_path, data_name, format_str, args,delimiter='\t', files=None, has_edge_importance=False, test_mode='test-dev'):
    if format_str == 'built_in':
        if data_name == 'Freebase':
            ...
            print(data_path)
            dataset = KGDatasetWiki(data_path,args, test_mode=test_mode)
        else:
