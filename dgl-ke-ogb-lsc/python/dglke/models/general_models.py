...
class InferModel(object):
    def __init__(self, device, model_name, hidden_dim,
        ...
        # 在这里注册新的embedding模型名字
        elif model_name == "Auto":
            self.score_func = AutoScore()
        elif model_name == "OTE":
            self.score_func = OTEScore()

    def load_emb(self, path, dataset,index):
        """Load the model.
        ...
        """
        self.entity_emb.load(path, dataset+'_'+self.model_name+'_entity_'+index)
        self.relation_emb.load(path, dataset+'_'+self.model_name+'_relation_'+index)
...
class KEModel(object):
    ...
    def save_emb(self, path, dataset,index):
        """Save the model.
        ...
        if self.encoder_model_name in ['shallow', 'concat', 'concat_v1']:
            if self.LRE:
                self.entity_emb_base.save(path, dataset+'_'+self.model_name+'_entity_emb_base_'+str(index))
                self.entity_emb_index.save(path, dataset+'_'+self.model_name+'_entity_emb_index_'+str(index))
            else:
                self.entity_emb.save(path, dataset+'_'+self.model_name+'_entity_'+str(index))
        if self.encoder_model_name in ['roberta', 'concat', 'concat_v1']:
            torch.save({'transform_state_dict': self.transform_net.state_dict()},
                       os.path.join(path, dataset+"_"+self.model_name+"_mlp_"+str(index)))
        if self.strict_rel_part or self.soft_rel_part:
            self.global_relation_emb.save(path, dataset+'_'+self.model_name+'_relation_'+str(index))
        else:
            if self.encoder_model_name in ['shallow', 'concat', 'concat_v1']:
                self.relation_emb.save(path, dataset+'_'+self.model_name+'_relation_'+str(index))
